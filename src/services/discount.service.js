...
class DiscountService {
    ...
    static async getDiscountAmount({ code, userId, shopId, products, applyDiscount = false }) {
        if (products.length === 0) throw new BadRequestError('Products is empty')
        const foundDiscount = await findDiscountByCode(code, shopId)
        console.log("🚀 ~ DiscountService ~ applyDiscountCode ~ foundDiscount:", foundDiscount)
        if (!foundDiscount) throw new BadRequestError('Discount not exists')
        const {
        ...
        console.log("🚀 ~ DiscountService ~ applyDiscountCode ~ discount_users_used:", discount_users_used)

        validateDiscount(foundDiscount)

        const totalOrder = await calculateTotalOrder(products)
        ...
        const { amount, totalPrice } = calculateDiscountAmount(discount_type, foundDiscount.discount_value, discount_max_value, totalOrder)
        if (applyDiscount) {
            await updateDiscountUsedCount(foundDiscount, userId)
        }
        ...
        return {
            ...
            totalOrder,
            type: discount_type,
            value: getDiscountValue(foundDiscount, discount_type),
            discount: amount,
